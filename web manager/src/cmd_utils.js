...
var pwdfile = path.join(process.cwd(), 'passwd');
...
exports.change_password = function(req, res) {
    ...
    if ( pwd != req.body['oldpwd'] ) {
        ...
        console.log(pwd);
		logger.error("change password failed(password not match)");
...
exports.stop_server = function (req, res) {
    ...
    var cmdstr = get_exec('stop');
    console.log(cmdstr);
    child_process.exec(cmdstr, {timeout: 1000},
        function (error, stdout, stderr) {
            if (error !== null) {
                fail_response(res, error, null);
				logger.info("stop service error:" + error);
            } else {
                console.log(cmdstr);
                succ_response(res, {});
...
var restart_service = function () {
    return new Promise((resolve, reject) => {
        var cmdstr = get_exec('restart');
        child_process.exec(cmdstr, {timeout: 1000},
            async function (error, stdout, stderr) {
                if (error !== null) {
                    reject(error);
					logger.info("restart service error:" + error);
                } else {                
                    resolve(stdout);
...
var restore_service = function () {
    return new Promise((resolve, reject) => {
        var cmdstr = get_exec('restore');
        child_process.exec(cmdstr, {timeout: 1000},
            async function (error, stdout, stderr) {
                if (error !== null) {
                    reject(error);
					logger.info("restore service error:" + error);
                } else {                
                    resolve(stdout);
					logger.info("restore service success");
                }
            });
        });
}
...
exports.set_lang = function (req, res) {
    ...
    if ( req.query['cmd'] == 'add' )  {
        var lang = req.body;
        var result = '{"lang":[]}';
        var langpath = path.join(process.cwd(), "html/lang/lang.json");
        var langfile = path.join(process.cwd(), "html/lang/" + lang['file']);
        ...
        succ_response ( res, {} ) ;
		logger.info("add language success(" + langpath + ")");
    // 删除语言包
        ...
        fs.writeFileSync(langpath, JSON.stringify(index, "utf-8"));     
        succ_response ( res, {} ) ;
		logger.info("del language success(" + langpath + ")");
    // 从索引文件内删除
    } else {
...
exports.get_lang = function (req, res) {
    if ( req.query['file'] ) {
        var langfile = path.join(process.cwd(), "html/lang/" + req.query['file']);
        var langdata = fs.readFileSync(langpath);
        succ_response ( res, langdata ) ;
    } else {
        var langpath = path.join(process.cwd(), "html/lang/lang.json");
...
exports.set_license = function (req, res) {
    var url = get_url(config.media.license);
    co(function* () {
        console.log(req.body);
        const result = yield req_smart_rtmpd("POST", url, req.body);
        if (result["code"] == 1) {
...
exports.get_license = function (req, res) {
    ...
    co(function* () {
        const result = yield req_smart_rtmpd("GET", url);
        if (result["code"] == 1) {
			logger.info('get_license1');
            fail_response( res, result["msg"] );
        } else {
			logger.info('get_license2');
            succ_response( res, JSON.parse(result["msg"]) );
        }
    });
}