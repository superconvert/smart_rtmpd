...
layui.define(["element","laytpl" ,"jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        compileMenu: function(menu,isSub){
            var menuHtml = '<li {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} class="layui-nav-item menu-li {{d.childOpenClass}} {{d.className}}"  {{#if( d.id){ }}  id="{{d.id}}" {{#}}}> <a {{#if( d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}} href="javascript:;">{{#if( d.icon){ }}  <i class="{{d.icon}}"></i> {{#}}} <span class="layui-left-nav i18n" data-lang="{{d.id}}">{{d.title}}</span></a>  {{# if(d.children){}} {{d.children}} {{#}}} </li>' ;
            if(isSub){
        ...
        listen: function () {
             ...
             * 菜单缩放
             */
            $('body').on('click', '.layuimini-site-mobile, .btn-menu-toggle', function () {
                var loading = layer.load(0, {shade: false, time: 2 * 1000});
